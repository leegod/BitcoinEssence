...
#if defined(NDEBUG)
# error "BitcoinEssence cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb8ab4b926fa27fb38c82601cc9a7aa461048daf61a2f696ba681333baae787d0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitcoinEssence: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitcoinEssence Signed Message:\n";
...
{
    // BitcoinEssence: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitcoinEssence: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitcoinEssence: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x667d69452cfb0ea2b907c1e3799ce72cc3063a3b29c729fbf5023c1b2e21e6e4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Here is earth, 2018-02-13.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cad2faf1709f6b6ca405427ef42a608c9331e1dc16863940cd11ec041ffdcc9c22bdd140dd5153fcb0dd582564bf17f6ab04d65c59fc7ebfaf9f20093d67d28f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518521681;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088187651;

        if (fTestNet)
        {
            block.nTime    = 1518521658;
            block.nNonce   = 388405675;   
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x137c575bcb9ea8b34bc5ed6524e041eacc628b3b80075208da901132a02d55f8"));    
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc3, 0xb2, 0xd3 }; // BitcoinEssence: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinEssenceMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinEssenceMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinEssenceMiner(CWallet *pwallet)
{
    printf("BitcoinEssenceMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinessence-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinEssenceMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinEssenceMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinEssenceMiner, pwallet));
}
